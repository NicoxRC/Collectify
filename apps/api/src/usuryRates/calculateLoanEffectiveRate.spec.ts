...
describe('calculateMaxEffectiveInstallmentRate', () => {
  ...
  it('picks the installment with the highest effective rate, not the first or last', () => {
    const lowInstallment: ConceptAssignment = {
      conceptTypeId: 'concept-1',
      name: 'Interés remuneratorio',
      calculationType: ConceptCalculationType.Percentage,
      value: 1,
    };
    const highInstallment: ConceptAssignment = {
      conceptTypeId: 'concept-1',
      name: 'Interés remuneratorio',
      calculationType: ConceptCalculationType.Percentage,
      value: 8,
    };
    const schedule = generateAmortizationSchedule(300000, 3, [
      [lowInstallment],
      [highInstallment],
      [lowInstallment],
    ]);

    expect(calculateMaxEffectiveInstallmentRate(schedule, 300000)).toBe(8);
  });
